	...
	public function getProducts() {
		...
		if(isset($products) && count($products)) {			
            foreach ($products as $key => $product) {
				...
				$productHTML .= '<form action="singleproduct2.php" method="POST" enctype="multipart/form-data">';
				$productHTML .= '<input type="hidden" name="p_id" value="'.$product['id'].'">';
				$productHTML .= '<button type="submit" class="btn" name="prodetail">';
                $productHTML .= '<div class="thumbnail product">';
                $productHTML .= '<figure>';
                $productHTML .= '<a href="#"><img width="300" height="380"  src="images/'.$product['images'].'" alt="'.$product['name'].'" /></a>';
                $productHTML .= '</figure>';
                $productHTML .= '<div class="caption">';
                $productHTML .= '<a href="singleproduct.php?id=$'.$product['id'].'"  class="product-name">'.$product['name'].'</a>';
